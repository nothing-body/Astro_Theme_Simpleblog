...
Live Demo 刻意指向已搭建完成的正式網站，讓使用者查看完成效果；公開設定範例仍保留 `example.com` 與佔位 ID，避免洩漏私人部署資料。
...
公開版只保留不呼叫 API 的三語系靜態離站提示，不包含私人查詢 API、KV 綁定、同步 Worker、清單或秘密。目的網址存放在 URL fragment，提示頁不會向任何信譽檢測服務發出請求。
